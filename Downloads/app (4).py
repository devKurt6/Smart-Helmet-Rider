...
# Encryption setup - generates a key from the app secret
def get_encryption_key():
    """
    Get or generate encryption key for settings.
    Uses app secret_key to derive a consistent encryption key.
    ...
    kdf = PBKDF2(
        ...
        length=32,
        salt=b'helmet_gps_salt_2024',  # Fixed salt for consistency
        iterations=100000,
    )
...
def encrypt_setting(value):
    """
    Encrypt a setting value using Fernet symmetric encryption.
    This allows us to decrypt it later when needed (unlike password hashing).
    
    Args:
        value (str): Plain text value to encrypt
    
    Returns:
        str: Encrypted value (can be decrypted later)
    """
    from cryptography.fernet import Fernet
...
def decrypt_setting(encrypted_value):
    ...
    try:
        ...
        # Might be plain text (not encrypted), return as-is
...
def verify_setting(plain_value, encrypted_value):
    ...
    """
    try:
        decrypted = decrypt_setting(encrypted_value)
        return plain_value == decrypted
    except:
        return False
...
def get_setting(key, default=None):
    ...
    Returns:
        str: Setting value (decrypted if encrypted) or default
    """
